...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val x = neighborsWithHistory(initial.head._1, initial.head._2)
      val y = newNeighborsOnly(x, explored)
      val z = explored ++ (y map (_._1))
      initial.head #:: from(initial.tail ++ y, z)
    }
